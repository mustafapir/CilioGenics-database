...
server <- function(input, output, session){
  ...
  genename<-reactive({
    if (toupper(trimmedGname()) %in% homsap$ensembl_gene_id){
      gname<-homsap$Gene_name[which(homsap$ensembl_gene_id %in% toupper(trimmedGname()))]
    }
    else if (trimmedGname() %in% homsap$entrez_id){
      gname<-homsap$Gene_name[which(homsap$entrez_id %in% trimmedGname())]
    }
    else {
      if (!(toupper(trimmedGname()) %in% toupper(homsap$Gene_name)) &&
        ...
        gname<-homsap$Gene_name[which(toupper(homsap$`Alias symbol`) %in% toupper(trimmedGname()))]
      }
  ...
  output$pubselecttable2<-renderReactable({
    reactable(pub_list(), resizable = FALSE, filterable = FALSE,
  ...
  gene_pub_list<-reactive({
    ...
    pubgene<-tbl(db, "publications") %>%
      filter(Publication == local(pubselecttable_selection())) %>%
